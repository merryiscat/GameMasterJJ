...
사용법:
    uv run python -m scripts.rules.run_pipeline --init              # 파이프라인 레코드 초기화
    uv run python -m scripts.rules.run_pipeline                     # 전체 실행
    uv run python -m scripts.rules.run_pipeline --rule-id 1         # 특정 룰만
    uv run python -m scripts.rules.run_pipeline --step ocr          # 특정 스텝만
    uv run python -m scripts.rules.run_pipeline --rule-id 1 --step parse  # 특정 룰의 특정 스텝
"""

import argparse
import sys

from scripts.rules import STEPS, db
from scripts.rules.step1_collect import process_collect
from scripts.rules.step2_translate import process_translate
from scripts.rules.step3_parse import process_parse
from scripts.rules.step4_llm_preprocess import process_llm_preprocess
from scripts.rules.step5_llm_qa import process_llm_qa
from scripts.rules.step6_vectorize import process_vectorize
...
def add_source(rule_id: int, source_type: str, url: str = None, file: str = None):
    """소스 수동 등록"""
    from scripts.rules.collectors import SOURCE_PRIORITY
